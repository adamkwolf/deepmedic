...
def padImageWithMirroring(inputImage, voxelsPerDimToPad):
    ...
    padLeft = int(voxelsPerDimToPad[0] // 2);
    padRight = int((voxelsPerDimToPad[0] + 1) // 2);
    paddedImage = T.concatenate([inputImage[:, :, int(voxelsPerDimToPad[0] // 2) - 1::-1, :, :], inputImage],
                                axis=2) if padLeft > 0 else inputImage
    paddedImage = T.concatenate([paddedImage, paddedImage[:, :, -1:-1 - int((voxelsPerDimToPad[0] + 1) // 2):-1, :, :]],
                                axis=2) if padRight > 0 else paddedImage
    # c-axis
    padLeft = int(voxelsPerDimToPad[1] // 2);
    padRight = int((voxelsPerDimToPad[1] + 1) // 2);
    paddedImage = T.concatenate([paddedImage[:, :, :, padLeft - 1::-1, :], paddedImage],
                                axis=3) if padLeft > 0 else paddedImage
    paddedImage = T.concatenate([paddedImage, paddedImage[:, :, :, -1:-1 - padRight:-1, :]],
                                axis=3) if padRight > 0 else paddedImage
    # z-axis
    padLeft = int(voxelsPerDimToPad[2] // 2);
    padRight = int((voxelsPerDimToPad[2] + 1) // 2)
    paddedImage = T.concatenate([paddedImage[:, :, :, :, padLeft - 1::-1], paddedImage],
                                axis=4) if padLeft > 0 else paddedImage
    paddedImage = T.concatenate([paddedImage, paddedImage[:, :, :, :, -1:-1 - padRight:-1]],
                                axis=4) if padRight > 0 else paddedImage
...
class Cnn3d(object):
    def __init__(self):
        ...
        self.learning_rate = theano.shared(
            np.cast["float32"](0.01))  # initial value, changed in make_cnn_model().compileTrainingFunction()
        ...
        self.inputTensorNormTrain = None
        self.inputTensorNormVal = None
        self.inputTensorNormTest = None
        self.listInputTensorPerSubsTrain = []
        self.listInputTensorPerSubsVal = []
        self.listInputTensorPerSubsTest = []

    def getNumSubsPathways(self):
        count = 0
        for pathway in self.pathways:
            if pathway.p_type() == pt.SUBS:
                count += 1
        return count
    ...
    def change_learning_rate_of_a_cnn(self, newValueForLearningRate, myLogger=None):
        stringToPrint = "UPDATE: (epoch-cnn-trained#" + str(
            self.numberOfEpochsTrained) + ") Changing the Cnn's Learning Rate to: " + str(newValueForLearningRate)
        if myLogger != None:
    ...
    def change_momentum_of_a_cnn(self, newValueForMomentum, myLogger=None):
        stringToPrint = "UPDATE: (epoch-cnn-trained#" + str(
            self.numberOfEpochsTrained) + ") Changing the Cnn's Momentum to: " + str(newValueForMomentum)
        if myLogger != None:
            myLogger.print3(stringToPrint)
        else:
            print(stringToPrint)
    ...
    def changeB1AndB2ParametersOfAdam(self, b1ParamForAdam, b2ParamForAdam, myLogger):
        myLogger.print3("UPDATE: (epoch-cnn-trained#" + str(
            self.numberOfEpochsTrained) + ") Changing the Cnn's B1 and B2 parameters for ADAM optimization to: B1 = " + str(
            b1ParamForAdam) + " || B2 = " + str(b2ParamForAdam))
        self.b1_adam = b1ParamForAdam
        self.b2_adam = b2ParamForAdam

    def changeRhoParameterOfRmsProp(self, rhoParamForRmsProp, myLogger):
        myLogger.print3("UPDATE: (epoch-cnn-trained#" + str(
            self.numberOfEpochsTrained) + ") Changing the Cnn's Rho parameter for RMSProp optimization to: Rho = " + str(
            rhoParamForRmsProp))
    ...
    def checkMeanValidationAccOfLastEpochAndUpdateCnnsTopAccAchievedIfNeeded(self,
        ...
        if meanValidationAccuracyOfLastEpoch > highestAchievedValidationAccuracyOfCnn + minIncreaseInValidationAccuracyConsideredForLrSchedule:
            ...
            myLogger.print3("UPDATE: In this last epoch (cnnTrained) #" + str(
                self.topMeanValidationAccuracyAchievedInEpoch[
                    1]) + " the CNN achieved a new highest mean validation accuracy of: " + str(
                self.topMeanValidationAccuracyAchievedInEpoch[0]))
    ...
    def _initializeSharedVarsForInputsTrain(self):
        ...
        for subsPath_i in xrange(self.numSubsPaths):
            self.sharedInpXPerSubsListTrain.append(
                theano.shared(np.zeros([1, 1, 1, 1, 1], dtype="float32"), borrow=self.borrowFlag))
    ...
    def _initializeSharedVarsForInputsVal(self):
        ...
        for subsPath_i in xrange(self.numSubsPaths):
            self.sharedInpXPerSubsListVal.append(
                theano.shared(np.zeros([1, 1, 1, 1, 1], dtype="float32"), borrow=self.borrowFlag))
    ...
    def _initializeSharedVarsForInputsTest(self):
        ...
        for subsPath_i in xrange(self.numSubsPaths):
            self.sharedInpXPerSubsListTest.append(
                theano.shared(np.zeros([1, 1, 1, 1, 1], dtype="float32"), borrow=self.borrowFlag))
    ...
    def _initializeSharedVariablesOfRmsProp(self, paramsToOptDuringTraining):
        ...
        self.velocities_forMom = []
        for param in paramsToOptDuringTraining:
            accu = theano.shared(param.get_value() * 0.,
                                 broadcastable=param.broadcastable)  # accumulates the mean of the grad's square.
    ...
    def getUpdatesAccordingToRmsProp(self, cost, params):
        ...
        for param, grad, accu, v in zip(params, grads, self.accuGradSquare_listForAllParamsRmsProp,
                                        self.velocities_forMom):
    ...
    def _getUpdatesOfTrainableParameters(self, myLogger, cost):
        ...
        paramsToOptDuringTraining = self._getTrainableParameters(myLogger)
        if self.sgd0orAdam1orRmsProp2 == 0:
            myLogger.print3("Optimizer used: [SGD]. Momentum used: Classic0 or Nesterov1 : " + str(
                self.classicMomentum0OrNesterov1))
            ...
            myLogger.print3("Optimizer used: [RMSProp]. Momentum used: Classic0 or Nesterov1 : " + str(
                self.classicMomentum0OrNesterov1))
    ...
    def _getTrainableParameters(self, myLogger):
        ...
        for pathway in self.pathways:
            for layer_i in xrange(0, len(pathway.getLayers())):
                if layer_i not in self.indicesOfLayersPerPathwayTypeToFreeze[pathway.p_type()]:
                    paramsToOptDuringTraining = paramsToOptDuringTraining + pathway.getLayer(
                        layer_i).getTrainableParams()
                else:  # Layer will be held fixed. Notice that Batch Norm parameters are still learnt.
                    myLogger.print3("WARN: [Pathway_" + str(pathway.getStringType()) + "] The weights of [Layer-" + str(
                        layer_i) + "] will NOT be trained as specified (index, first layer is 0).")
    ...
    def _getUpdatesForBnRollingAverage(self):
        ...
        for pathway in self.pathways:
            for layer in pathway.getLayers():
                updatesForBnRollingAverage.extend(
                    layer.getUpdatesForBnRollingAverage())  # CAREFUL: WARN, PROBLEM, THEANO BUG! If a layer has only 1FM, the .newMu_B ends up being of type (true,) instead of vector!!! Error!!!
    ...
    def compileTrainFunction(self, myLogger):
        ...
        if not self.checkTrainingStateAttributesInitialized():
            myLogger.print3(
                "ERROR: Prior to compiling the training function, training state attributes need to be initialized via a call of [Cnn3d.setTrainingStateAttributes(...)]. Exiting!");
            exit(1)
        ...
        if self.costFunctionLetter == "L":
            ...
            myLogger.print3(
                "ERROR: Problem in make_cnn_model(). The parameter self.costFunctionLetter did not have an acceptable value( L,D,J ). Exiting.");
            exit(1)
    ...
    def compileValidationFunction(self, myLogger):
        ...
        for subPath_i in xrange(self.numSubsPaths):  # if there are subsampled paths...
            ...
            givensSet.update(
                {xSub: sharedInpXSubVal[index * self.batchSizeValidation: (index + 1) * self.batchSizeValidation]})
        givensSet.update(
            {y: intCastSharedLabelsYVal[index * self.batchSizeValidation: (index + 1) * self.batchSizeValidation]})
    ...
    def compileTestAndVisualisationFunction(self, myLogger):
        ...
        listToReturnWithAllTheFmActivationsAndPredictionsAppended.append(
            self.finalTargetLayer.predictionProbabilities())
        ...
        for subPath_i in xrange(self.numSubsPaths):  # if there are subsampled paths...
            ...
            givensSet.update(
                {xSub: sharedInpXSubTest[index * self.batchSizeTesting: (index + 1) * self.batchSizeTesting]})

        myLogger.print3(
            "...Compiling the function for testing and visualisation of FMs... (This may take a few minutes...)")
    ...
    def _getInputTensorsXToCnn(self):
        if not self.inputTensorsXToCnnInitialized:
            ...
            for subsPath_i in xrange(self.numSubsPaths):
                self.listInputTensorPerSubsTrain.append(
                    tensor5())  # Actually, for these 3, a single tensor5() could be used.
                self.listInputTensorPerSubsVal.append(
                    tensor5())  # myTensor.reshape(inputImageSubsampledShapeValidation)
    ...
    def make_cnn_model(self,
                       ...
                       # Controls how many pathways: [] if no secondary pathways. Else, List of lists. One sublist per secondary pathway. Each sublist has 3 ints, the rcz subsampling factors.
                       # === FC Layers ===
                       fcLayersFMs,
                       ...
                       # list of sublists, one for each pathway. Each either empty or full with the dropout rates of all the layers in the path.
                       # Initialization
                       convWInitMethod,
                       # Batch Normalization
        ...
        thisPathwayUseBnPerLayer[0] = applyBnToInputOfPathways[
            thisPathwayType] if rollingAverageForBatchNormalizationOverThatManyBatches > 0 else False  # For the 1st layer, ask specific flag.
        ...
        [dimsOfOutputFrom1stPathwayTrain, dimsOfOutputFrom1stPathwayVal,
         dimsOfOutputFrom1stPathwayTest] = thisPathway.getShapeOfOutput()
        ...
        for subPath_i in xrange(self.numSubsPaths):
            ...
            self.pathways.append(
                thisPathway)  # There will be at least an entry as a secondary pathway. But it won't have any layers if it was not actually used.
            thisPathwayType = thisPathway.pType()
            ...
            thisPathwayNumOfLayers = len(thisPathWayNKerns)
            thisPathwayUseBnPerLayer = [
                                           rollingAverageForBatchNormalizationOverThatManyBatches > 0] * thisPathwayNumOfLayers
            thisPathwayUseBnPerLayer[0] = applyBnToInputOfPathways[
                thisPathwayType] if rollingAverageForBatchNormalizationOverThatManyBatches > 0 else False  # For the 1st layer, ask specific flag.
            ...
            inputToPathwayShapeTrain = [self.batchSize,
                                        numberOfImageChannelsPath2] + thisPathway.calcInputRczDimsToProduceOutputFmsOfCompatibleDims(
                thisPathWayKernelDimensions, dimsOfOutputFrom1stPathwayTrain);
            inputToPathwayShapeVal = [self.batchSizeValidation,
                                      numberOfImageChannelsPath2] + thisPathway.calcInputRczDimsToProduceOutputFmsOfCompatibleDims(
                thisPathWayKernelDimensions, dimsOfOutputFrom1stPathwayVal)
            inputToPathwayShapeTest = [self.batchSizeTesting,
                                       numberOfImageChannelsPath2] + thisPathway.calcInputRczDimsToProduceOutputFmsOfCompatibleDims(
        ...
        for path_i in xrange(len(self.pathways)):
            [outputNormResOfPathTrain, outputNormResOfPathVal, outputNormResOfPathTest] = self.pathways[
                path_i].getOutputAtNormalRes()
            ...
            inputToFirstFcLayerTrain = T.concatenate([inputToFirstFcLayerTrain, outputNormResOfPathTrain],
                                                     axis=1) if path_i != 0 else outputNormResOfPathTrain
            inputToFirstFcLayerVal = T.concatenate([inputToFirstFcLayerVal, outputNormResOfPathVal],
                                                   axis=1) if path_i != 0 else outputNormResOfPathVal
            inputToFirstFcLayerTest = T.concatenate([inputToFirstFcLayerTest, outputNormResOfPathTest],
        ...
        myLogger.print3(
            "DEBUG: Shape of the kernel of the first FC layer is : " + str(firstFcLayerAfterConcatenationKernelShape))
        myLogger.print3("DEBUG: Input to the FC Pathway will be padded by that many voxels per dimension: " + str(
            voxelsToPadPerDim))
        ...
        inputToPathwayShapeTrain = [self.batchSize, numberOfFmsOfInputToFirstFcLayer] + dimsOfOutputFrom1stPathwayTrain[
                                                                                        2:5]
        inputToPathwayShapeVal = [self.batchSizeValidation,
                                  numberOfFmsOfInputToFirstFcLayer] + dimsOfOutputFrom1stPathwayVal[2:5]
        inputToPathwayShapeTest = [self.batchSizeTesting,
                                   numberOfFmsOfInputToFirstFcLayer] + dimsOfOutputFrom1stPathwayTest[2:5]
        ...
        thisPathWayKernelDimensions = [firstFcLayerAfterConcatenationKernelShape] + [[1, 1, 1]] * (
            len(thisPathWayNKerns) - 1)
